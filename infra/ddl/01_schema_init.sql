...
CREATE DATABASE /*!32312 IF NOT EXISTS*/ `aegis` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
...
CREATE TABLE `agent_api` (
  ...
  `api_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'API名称，展示用',
  `api_path` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'API路径，租户内唯一，格式 /api/v1/agents/{code}/invoke',
  `http_method` varchar(16) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'HTTP方法：GET/POST，默认POST',
  `auth_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '认证类型：API_KEY/BEARER/OAUTH2/BASIC/NONE',
  `response_mode` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '响应模式：SYNC/ASYNC/SSE',
  `rate_limit` int DEFAULT NULL COMMENT '限流QPS，每秒最大请求数',
  `timeout` int DEFAULT NULL COMMENT '超时时间（秒），默认30',
  `validity_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '有效期类型：PERMANENT（永久）/ DAYS_7（7天）/ DAYS_30（30天）/ CUSTOM（自定义）',
  `valid_until` datetime DEFAULT NULL COMMENT '固定有效期截止时间，validityType非PERMANENT时生效',
  `scope_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '数据出境范围类型：INTERNAL_IP（企业内部白名单IP）/ DEPT（指定部门）/ PARTNER（指定外部合作伙伴）',
  `scope_config` json DEFAULT NULL COMMENT '出境范围配置，JSON格式（含白名单域名/字段等）',
  `api_key` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'API密钥，仅创建时明文返回，存储哈希值',
  `webhook_url` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '回调地址，异步模式结果回调URL',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'API状态：NORMAL（正常）/ DISABLED（停用）',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `deleted` tinyint DEFAULT '0' COMMENT '逻辑删除：0未删除 1已删除',
  `deployment_pool_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '部署目标沙箱池编码（外键 sbx_pool.pool_code + tenantId），系统智能体必填',
  `reserved_replicas` int DEFAULT '1' COMMENT '该智能体在绑定池内的预留常驻副本数，保证对外 API 冷启动与容量',
  `version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT '1.0.0' COMMENT 'API 版本号，如 1.0.0',
  ...
  `api_doc_url` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'API 文档 URL',
  `last_tested_at` datetime DEFAULT NULL COMMENT '最近测试时间',
  `bearer_token_mode` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT 'PASSTHROUGH' COMMENT 'Bearer Token 管理模式：STATIC/PASSTHROUGH',
  `bearer_token_value` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '静态模式下的 Token 值',
  `bearer_jwt_secret` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'JWT 签名密钥/公钥',
  `bearer_jwt_algorithm` varchar(16) COLLATE utf8mb4_unicode_ci DEFAULT 'HS256' COMMENT 'JWT 签名算法',
  `bearer_introspection_url` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Token introspection 端点',
  `bearer_pass_through` tinyint(1) DEFAULT '0' COMMENT '是否将 Token 透传给下游 Agent 服务',
  `pool_allocate_status` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'sandbox allocate status PENDING/ALLOCATED/FAILED',
...
CREATE TABLE `agent_api_key` (
  ...
  `key_label` varchar(64) DEFAULT '主Key' COMMENT 'Key 标签',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='智能体 API Key 生命周期管理';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `agent_binding` (
  ...
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `agent_id` bigint DEFAULT NULL COMMENT '智能体ID，关联AgentDef主键',
  `agent_version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '智能体版本号，绑定归属的版本',
  `resource_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '资源类型：SKILL/KNOWLEDGE_BASE/MCP/TOOL/DATASET',
  `resource_id` bigint DEFAULT NULL COMMENT '资源ID，关联具体资源主键',
  `resource_version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '资源版本，固定绑定为具体版本号，动态绑定为latest',
  `binding_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '绑定类型：FIXED（固定绑定，版本锁定）/ DYNAMIC（动态加载，运行时解析）',
...
CREATE TABLE `agent_config` (
  ...
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `agent_id` bigint DEFAULT NULL COMMENT '智能体ID，关联AgentDef主键',
  `version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '配置版本号，与AgentDef.version对应',
  `system_prompt` text COLLATE utf8mb4_unicode_ci COMMENT '系统提示词，定义智能体角色与行为约束',
  `model_tier` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '模型档位：LIGHT（轻量）/ STANDARD（标准）/ STRONG（强力）',
  `temperature` decimal(20,4) DEFAULT NULL COMMENT '温度参数，0-2，值越高输出越发散，0为确定性输出',
  `memory_strategy` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '记忆策略：SESSION_LEVEL（会话级）/ LONG_TERM（长期，用户归档）',
...
CREATE TABLE `agent_def` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `agent_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '智能体编码，租户内唯一，创建后不可修改',
  `agent_name` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '智能体名称，展示用',
  `agent_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '智能体类型：UNIVERSAL（通用智能体，平台唯一）/ APPLICATION（应用智能体，用户创建）',
  `icon` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '智能体图标URL',
  `color` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '主题色，前端展示用，十六进制色值',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '智能体描述，介绍智能体能力与适用场景',
  `category` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '智能体分类，市场检索用',
  `life_status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '生命周期状态：DRAFT→REVIEWING→PUBLISHED→ARCHIVED',
  `version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '当前版本号，每次发布递增',
  ...
  `visibility` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT 'TENANT' COMMENT '发布可见范围：TENANT（本租户可见，默认）/ PUBLIC（全平台可见，跨租户市场）',
  ...
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `deleted` tinyint DEFAULT '0' COMMENT '逻辑删除：0未删除 1已删除',
  `usage_scenario` varchar(20) COLLATE utf8mb4_unicode_ci DEFAULT 'PERSONAL' COMMENT '使用场景：PERSONAL-个人使用/SHARED-共享发布',
  `governance_tier` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT 'STANDARD' COMMENT '治理档位：STANDARD-标准/ENHANCED-增强/STRICT-严格，取代安全级别/护栏级别/规划模式',
  PRIMARY KEY (`id`),
...
CREATE TABLE `agent_memory` (
  ...
  `memory_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '记忆类型：USER_PROFILE（用户画像）/ TASK_SUMMARY（任务摘要）/ KEY_FACT（关键事实）',
  `memory_key` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '记忆键，结构化标识，如 profile.role / fact.project_name',
  ...
  `source` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '记忆来源：auto-自动提取 manual-手动录入 import-外部导入',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `agent_subscription` (
  ...
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '订阅状态：ACTIVE（已订阅）/ UNSUBSCRIBED（已退订）',
...
CREATE TABLE `agent_workspace_material` (
  ...
  `isolation_scope` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '隔离作用域：USER / AGENT / GLOBAL',
  `workspace_path` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '工作区路径',
  `material_fingerprint` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '物化指纹（绑定资源版本号的hash）',
  `binding_snapshot` text COLLATE utf8mb4_unicode_ci COMMENT '绑定快照（JSON，记录当时的绑定列表）',
...
CREATE TABLE `eval_task` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `task_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '任务唯一标识，UUID字符串，用于全链路追踪',
  `agent_id` bigint DEFAULT NULL COMMENT '被评测智能体ID，关联agent_def.id',
  `agent_version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '智能体版本，评测时的智能体版本号，保证评测可复现',
  `suite_id` bigint DEFAULT NULL COMMENT '测试套件ID，关联test_suite.id，评测使用的测试集',
  `trigger_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '触发类型：PRE_RELEASE（版本发布前）/ MANUAL（手动）/ SCHEDULED（定时回归）',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '任务状态：COMPLETED（已完成）/ IN_PROGRESS（进行中）/ QUEUED（排队中）',
...
CREATE TABLE `eval_test_case` (
  ...
  `case_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '用例唯一标识，套件内唯一，用于用例检索与引用',
  `input_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '输入类型：TEXT（文本）/ FILE（文件）',
  `input_content` text COLLATE utf8mb4_unicode_ci COMMENT '输入内容，依据inputType不同结构不同，如纯文本或JSON多模态描述',
  `expected_output` text COLLATE utf8mb4_unicode_ci COMMENT '期望输出，用于与实际输出比对，可以是文本或JSON结构',
  `expected_tool` json DEFAULT NULL COMMENT '期望工具，JSON数组字符串，期望智能体调用的工具列表',
  `eval_method` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '评估方法：EXACT_MATCH（精确匹配）/ KEYWORD（关键词包含）/ LLM_SCORE（LLM评分）',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `eval_test_suite` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `suite_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '套件名称，长度不超过128，标识测试套件用途',
  ...
  `version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '版本号，语义化版本如1.0.0，支持套件的版本演进与回溯',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '套件描述，长度不超过512，说明套件覆盖场景与测试目标',
...
CREATE TABLE `model_def` (
  ...
  `model_code` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '模型唯一编码，提供商内唯一，如gpt-4、claude-3-opus、qwen-max',
  `model_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '模型展示名称，长度不超过128，如"GPT-4 Turbo"、"通义千问-Max"',
  `tier` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '模型层级：LIGHT（轻量）/ STANDARD（标准）/ STRONG（强力），用于路由选择',
  `model_type` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'TEXT' COMMENT '模型用途：TEXT/MULTIMODAL/EMBEDDING/VISION',
  ...
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '状态：ENABLED（启用）/ DISABLED（禁用），管理员控制模型可用性',
  `latency` decimal(20,4) DEFAULT NULL COMMENT '平均延迟，单位毫秒，由系统监控统计，用于路由决策',
  `quality_grade` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '质量等级：S_PLUS/S/A_PLUS/A/B_PLUS，综合评估模型输出质量',
...
) ENGINE=InnoDB AUTO_INCREMENT=1104 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='模型定义实体';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `model_provider` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `provider_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '提供商唯一编码，全局唯一，如openai、anthropic、qwen，长度不超过64',
  `provider_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '提供商展示名称，长度不超过128，如"OpenAI"、"通义千问"',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '状态：ACTIVE（已接入）/ PENDING（待接入），管理员控制提供商可用性',
  `endpoint` varchar(1024) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '服务接入端点，API基础URL，如https://api.openai.com/v1',
  `api_key` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'API密钥，敏感字段，存储时加密，用于服务端鉴权',
  ...
  `color` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '品牌颜色，十六进制色值如#10A37F，用于前端展示标识',
...
CREATE TABLE `model_rate_limit` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `scope` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '限流作用域：PLATFORM（全平台）/ DEPT（部门）/ USER（个人）',
  ...
  `action` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '超限动作：ALERT（告警）/ LIMIT（限流）/ PASS（放行）',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `model_route` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `tier` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '模型层级：LIGHT（轻量）/ STANDARD（标准）/ STRONG（强力），路由按层级匹配模型',
  ...
  `scope` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '限流作用域：PLATFORM（全平台）/ DEPT（部门）/ USER（个人）',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '路由描述，长度不超过512，说明路由策略适用场景与配置理由',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `mon_audit_log` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `log_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '日志类型：OPERATION（操作）/ SECURITY（安全）/ DATA（数据）/ SESSION（会话）/ TOOL（工具）/ EXPORT（导出）',
  `user_id` bigint DEFAULT NULL COMMENT '操作人用户ID，关联org_user.id',
  `username` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '操作人用户名，冗余存储便于审计列表展示',
  `operation` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '操作类型，如CREATE_AGENT/UPDATE_SKILL/DELETE_KB，标识具体操作',
  `resource_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '资源类型，如AGENT/SKILL/KNOWLEDGE_BASE，被操作资源种类',
  `resource_name` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '资源名称，被操作资源的名称，便于审计追溯',
  `detail` text COLLATE utf8mb4_unicode_ci COMMENT '操作详情，JSON字符串，记录操作的具体参数与变更内容',
  `result` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '操作结果：SUCCESS（成功）/ BLOCKED（拦截）/ ALERT（告警）/ RECORDED（已记录）',
  `ip` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '客户端IP，操作发起的IP地址，用于安全审计',
  `user_agent` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'User-Agent，客户端标识，记录操作来源设备与浏览器',
  `trace_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '链路追踪ID，用于全链路日志关联与排查',
...
CREATE TABLE `mon_backup_record` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `backup_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '备份ID',
  `backup_type` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'FULL/INCREMENTAL',
  `size_bytes` bigint DEFAULT '0' COMMENT '备份大小(字节)',
  `duration_sec` int DEFAULT '0' COMMENT '耗时(秒)',
  `status` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'SUCCESS/RUNNING/FAILED',
  `location` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '存储位置',
...
CREATE TABLE `mon_drill_record` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `drill_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '演练ID',
  `drill_type` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'FAILOVER/RECOVERY/FULLCHAIN',
  `duration_sec` int DEFAULT '0' COMMENT '耗时(秒)',
  `result` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'PASS/PARTIAL/FAIL',
  `report` mediumtext COLLATE utf8mb4_unicode_ci COMMENT '演练报告',
...
CREATE TABLE `mon_failover_record` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `reason` varchar(512) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '触发原因',
  `from_node` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '源节点',
  `to_node` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '目标节点',
  `duration_sec` int DEFAULT '0' COMMENT '耗时(秒)',
  `status` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'SUCCESS/FAILED/ROLLBACK',
  `trigger_type` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'AUTO/MANUAL',
  `occur_time` datetime NOT NULL COMMENT '发生时间',
  `tenant_id` bigint DEFAULT NULL COMMENT '租户ID',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `mon_span` (
  ...
  `trace_id` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '所属链路ID',
  `span_id` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'span标识',
  `parent_span_id` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '父span',
  `span_type` varchar(24) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'SPAN类型',
  `name` varchar(128) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '工具名/模型名/操作名',
  ...
  `status` varchar(16) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'SUCCESS' COMMENT 'SUCCESS/FAILED/SKIPPED',
  `start_time` datetime NOT NULL COMMENT '开始时间',
  `end_time` datetime DEFAULT NULL COMMENT '结束时间',
  `duration_ms` int DEFAULT NULL COMMENT '耗时(ms)',
  `input_summary` varchar(1024) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '入参摘要',
  `output_summary` varchar(1024) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '出参摘要',
  ...
  `error_msg` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '失败详情',
  `meta` text COLLATE utf8mb4_unicode_ci COMMENT '扩展属性JSON',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `create_by` bigint DEFAULT NULL COMMENT 'creator id',
  `update_by` bigint DEFAULT NULL COMMENT 'updater id',
...
CREATE TABLE `mon_trace` (
  `id` bigint NOT NULL COMMENT '主键',
  `trace_id` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '链路ID',
  `session_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '所属会话ID',
  `agent_id` bigint DEFAULT NULL COMMENT '执行智能体ID',
  `agent_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '智能体名称',
  `user_id` bigint DEFAULT NULL COMMENT '发起用户ID',
  `user_name` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '用户名',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `api_path` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '入口路径',
  `status` varchar(16) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'RUNNING' COMMENT 'RUNNING/SUCCESS/FAILED/TIMEOUT',
  `start_time` datetime NOT NULL COMMENT '开始时间',
  `end_time` datetime DEFAULT NULL COMMENT '结束时间',
...
CREATE TABLE `org_department` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `dept_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '部门名称，租户内可重复，同级建议唯一',
  `parent_id` bigint DEFAULT NULL COMMENT '父部门ID，根部门为0或null',
  `dept_path` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '部门完整路径，格式 /root/parent/self/，用于祖先/后代查询',
  ...
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '部门状态：NORMAL（正常）/ DISABLED（禁用），禁用后不在组织树展示',
  `sync_source` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '同步来源：HR（HR同步）/ OA（OA同步）/ LDAP（LDAP同步）/ MANUAL（手动创建）',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `org_role` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `role_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '角色编码，租户内唯一，程序引用标识，创建后不可修改',
  `role_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '角色名称，展示用',
  `role_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '角色类型：PLATFORM（平台角色，系统操作权限）/ RESOURCE（资源角色，资源访问权限）',
  `parent_role_id` bigint DEFAULT NULL COMMENT '父角色ID，用于角色继承，null表示无父角色',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '角色描述，说明角色职责与权限范围',
  `sort` int DEFAULT NULL COMMENT '同级排序号，升序排列',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '角色状态：NORMAL（正常）/ DISABLED（禁用），禁用后关联用户失去该角色权限',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `org_user` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `username` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '用户名，租户内唯一，登录凭证，创建后不可修改',
  `password` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '密码哈希，BCrypt加密存储，不可逆向',
  `real_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '真实姓名，展示用',
  `emp_no` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '工号，租户内唯一，用于与HR系统对接',
  `email` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '邮箱，用于通知与密码找回，租户内唯一',
  `phone` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '手机号，用于MFA与紧急通知，租户内唯一',
  `avatar` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '头像URL',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '用户状态：NORMAL（正常）/ DISABLED（禁用），禁用后无法登录',
  `last_login_time` datetime DEFAULT NULL COMMENT '最后登录时间',
  `last_login_ip` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '最后登录IP，用于安全审计',
...
CREATE TABLE `org_user_role` (
  ...
  `resource_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '资源类型，资源角色时必填，标识资源类别',
  `source` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '授权来源：DIRECT（直接授予）/ DEPT_INHERIT（部门继承）/ RESOURCE_AUTH（资源授权）',
...
CREATE TABLE `org_permission` (
  ...
  `permission_code` varchar(128) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '权限编码，如 agent:view / tenant:manage，租户内唯一，程序引用标识',
  `permission_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '权限名称，展示用',
  `permission_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '权限类型：MENU（菜单）/ BUTTON（按钮）/ API（接口）',
  `parent_id` bigint DEFAULT 0 COMMENT '父权限ID，用于权限树展示，0表示根节点',
  `sort` int DEFAULT 0 COMMENT '同级排序号，升序排列',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT 'NORMAL' COMMENT '状态：NORMAL（正常）/ DISABLED（禁用）',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `res_kb_document` (
  ...
  `file_name` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '文件名，包含扩展名，长度不超过255',
  `file_type` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '文件类型，取值：PDF/DOCX/XLSX/PPTX/TXT/MD/HTML等',
  `file_size` bigint DEFAULT NULL COMMENT '文件大小，单位字节',
  `oss_key` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'OSS存储键，文件在对象存储中的唯一路径',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '处理状态：PENDING（待扫描）/ SCANNING（扫描中）/ CHUNKING（切片中）/ CHUNKED（已切片）/ FAILED（失败）',
  `chunk_count` int DEFAULT NULL COMMENT '切片数量，文档切分后的总片段数',
  `permission_level` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '权限级别：CREATOR（仅创建者）/ DEPT（同部门）/ ALL（全员可查看）',
...
CREATE TABLE `res_kb_document_chunk` (
  ...
  `content` text COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '切片文本内容',
...
CREATE TABLE `res_kb_process_progress` (
  ...
  `doc_id` bigint NOT NULL COMMENT '所属文档ID，关联res_kb_document.id',
  `step` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '处理步骤：DOWNLOADING（下载中）/ SCANNING（扫描中）/ CHUNKING（切片中）/ EMBEDDING（嵌入中）/ VECTORING（向量入库中）/ COMPLETED（已完成）/ FAILED（失败）',
  `step_order` int NOT NULL COMMENT '步骤顺序，从1开始，用于排序',
  `status` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '步骤状态：PENDING（待执行）/ RUNNING（执行中）/ COMPLETED（已完成）/ FAILED（失败）',
  `progress_percent` int DEFAULT NULL COMMENT '当前步骤进度百分比，0-100',
  `message` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '步骤描述信息，如"下载MinIO文件..."、"切片 3/10"等',
  `started_at` datetime DEFAULT NULL COMMENT '步骤开始时间',
  `completed_at` datetime DEFAULT NULL COMMENT '步骤完成时间',
  `error_detail` varchar(1024) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '错误详情，失败时记录异常信息',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `res_kb_subscription` (
  ...
  `kb_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `subscriber_type` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL,
...
CREATE TABLE `res_knowledge_base` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `kb_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '知识库唯一编码，租户内唯一，由字母、数字、下划线组成，长度不超过64',
  `kb_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '知识库展示名称，长度不超过128',
  `icon` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '知识库图标URL，可选',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '知识库描述，长度不超过512，说明知识库内容范围与适用场景',
  `security_level` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '安全等级：L1~L4，影响知识库可见范围与检索权限',
  `life_status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '生命周期状态：DRAFT→REVIEWING→PUBLISHED→ARCHIVED',
  `version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '版本号，语义化版本如1.0.0',
  ...
  `chunk_strategy` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '切片策略，取值：FIXED（固定长度）/ SENTENCE（按句）/ PARAGRAPH（按段）/ MARKDOWN（按标题）',
  ...
  `embedding_model` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '嵌入模型标识，如doubao-embedding-vision、bge-large-zh、text-embedding-3-large，取值需为模型管理中已启用的EMBEDDING模型',
  `retrieval_strategy` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT 'VECTOR' COMMENT '检索策略，取值：VECTOR（向量检索）/ KEYWORD（关键词）/ HYBRID（混合检索）',
...
CREATE TABLE `res_mcp_service` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `mcp_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'MCP服务唯一编码，全局唯一，由字母、数字、下划线组成，长度不超过64',
  `mcp_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'MCP服务展示名称，长度不超过128',
  `icon` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '服务图标URL，可选',
  `provider` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '服务提供方，如"官方"、"合作厂商"，标识服务来源',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '服务描述，长度不超过512，说明服务能力与适用场景',
  `version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '版本号，语义化版本如1.0.0',
  `endpoint` varchar(1024) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '服务接入端点，SSE/HTTP协议为URL，STDIO协议为可执行命令路径',
  `protocol` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '传输协议：STDIO（标准输入输出）/ SSE（Server-Sent Events）/ STREAMABLE_HTTP（可流式HTTP）',
  `auth_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '鉴权类型：API_KEY/BEARER/OAUTH2/BASIC/NONE，决定authConfig结构',
  ...
  `security_level` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '安全等级：L1~L4，影响服务可用范围与调用权限',
  `subs_count` int DEFAULT NULL COMMENT '订阅数，该服务被订阅的总次数',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '状态：ACTIVE（已接入）/ PENDING（待接入），管理员控制服务可用性',
  `life_status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '生命周期状态：DRAFT（草稿）/ REVIEWING（审核中）/ PUBLISHED（已发布）/ ARCHIVED（已归档）/ REJECTED（已驳回），管理审核状态机',
  ...
  `deleted` tinyint DEFAULT '0' COMMENT '逻辑删除：0未删除 1已删除',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_mcp_service_code` (`mcp_code`),
...
CREATE TABLE `res_mcp_subscription` (
  ...
  `mcp_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'MCP服务编码，冗余存储便于查询',
  `subscriber_type` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '订阅者类型：USER（用户）/ AGENT（智能体）',
...
CREATE TABLE `res_review` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `resource_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '资源类型，标识被审核资源种类',
  `resource_id` bigint DEFAULT NULL COMMENT '资源ID，关联对应资源表的主键',
  `resource_name` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '资源名称，冗余存储便于审核列表展示',
  `resource_version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '资源版本号，标识被审核的版本',
  ...
  `review_status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '审核状态：PENDING（待审核）/ APPROVED（已通过）/ REJECTED（已拒绝）',
  ...
  `reject_reason` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '驳回原因，当reviewStatus为REJECTED时填写',
...
CREATE TABLE `res_review_node` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `review_chain_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '审批链ID（同一链多节点按 node_order 串联）',
  `node_order` int NOT NULL COMMENT '审批节点顺序，从 1 开始',
  `approver_type` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '审批人类型：ROLE（角色）/ USER（指定人）/ DEPT_LEADER（部门主管）',
  `approver_ref` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '审批人引用：角色编码 / 用户ID / 部门ID',
  `node_status` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT 'PENDING' COMMENT '节点状态：PENDING / APPROVED / REJECTED / SKIPPED',
...
CREATE TABLE `res_skill` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `skill_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '技能唯一编码，租户内唯一，由字母、数字、下划线组成，长度不超过64',
  `skill_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '技能展示名称，长度不超过128，用于资源中心与智能体配置页展示',
  `icon` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '技能图标URL，可选，用于资源中心视觉标识',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '技能描述，长度不超过512，向用户说明技能用途与适用场景',
  `skill_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '技能类型：ATOMIC（原子技能）/ COMPOSITE（组合技能）',
  `category` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '技能分类：数据处理/内容生成/集成对接/计算/检索',
  `tags` json DEFAULT NULL COMMENT '技能标签，JSON数组格式（如 ["推荐","官方"]），用于检索与筛选',
  `security_level` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '安全等级：L1~L4，影响技能可用范围与数据访问权限',
  `life_status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '生命周期状态：DRAFT→REVIEWING→PUBLISHED→ARCHIVED',
  `version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '技能版本号，语义化版本如1.0.0，发布后只增不减',
  `author_user_id` bigint DEFAULT NULL COMMENT '创建者用户ID，关联org_user.id',
  ...
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `deleted` tinyint DEFAULT '0' COMMENT '逻辑删除：0未删除 1已删除',
  `instructions` text COLLATE utf8mb4_unicode_ci COMMENT '技能方法论正文（SKILL.md body），V2 核心：承载可执行的操作范式，是创建痛点修复的关键列',
  ...
  `active_version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '当前生效版本指针（指针式发布，回滚=改此列），对应 res_skill_version.version',
  `canary_version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '灰度版本指针，NULL 表示无灰度',
  `latest_version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '最新版本号（含草稿态）',
  ...
  `scope` varchar(20) COLLATE utf8mb4_unicode_ci DEFAULT 'LOCAL' COMMENT '作用域：GLOBAL=全局（所有用户自动加载）/ LOCAL=局部（权限过滤）',
  `exec_config` text COLLATE utf8mb4_unicode_ci COMMENT 'exec config JSON: model tier/temperature/maxTurns/guardrails',
...
CREATE TABLE `res_skill_subscription` (
  ...
  `skill_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '技能编码（冗余）',
  `subscriber_type` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '订阅者类型：USER（用户订阅）/ AGENT（智能体订阅）',
  `subscriber_id` bigint NOT NULL COMMENT '订阅者ID：用户ID 或 智能体ID',
  `subscribed_version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '锁定订阅版本，NULL 表示跟随 active_version',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `deleted` tinyint DEFAULT '0' COMMENT '逻辑删除',
  `update_by` bigint DEFAULT NULL COMMENT 'updater id',
...
CREATE TABLE `res_skill_version` (
  ...
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `skill_id` bigint NOT NULL COMMENT '关联 res_skill.id',
  `skill_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '技能编码（冗余，便于查询）',
  `version` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '快照版本号，语义化如 1.0.0',
  `skill_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '技能展示名称（快照）',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '技能描述（快照）',
  `category` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '分类（快照）',
  `tags` json DEFAULT NULL COMMENT '标签（快照）',
  `security_level` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '安全等级（快照）',
  `instructions` text COLLATE utf8mb4_unicode_ci COMMENT '方法论正文（快照，不可变）',
  ...
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `deleted` tinyint DEFAULT '0' COMMENT '逻辑删除',
  `debug_report` mediumtext COLLATE utf8mb4_unicode_ci COMMENT '调试报告 JSON',
  `security_report` mediumtext COLLATE utf8mb4_unicode_ci COMMENT '安全扫描报告 JSON',
  `package_url` varchar(500) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '技能压缩包下载地址',
  `quality_score` decimal(3,1) DEFAULT NULL COMMENT '质量评分（0-10）',
  `update_by` bigint DEFAULT NULL COMMENT 'updater id',
  `update_time` datetime DEFAULT NULL COMMENT 'update time',
  `exec_config` text COLLATE utf8mb4_unicode_ci COMMENT 'exec config snapshot JSON',
  PRIMARY KEY (`id`),
...
CREATE TABLE `res_tool` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tool_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '工具唯一编码，全局唯一，由字母、数字、下划线组成，长度不超过64',
  `tool_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '工具展示名称，长度不超过128',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '工具描述，长度不超过512，说明工具能力与调用场景',
  `tool_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '工具类型：READONLY/INTERNAL_API/WRITE/EXTERNAL_NETWORK/CODE_EXEC/HIGH_RISK',
  `source_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '来源类型：BUILTIN（平台内置）/ MCP（MCP工具）',
  ...
  `security_level` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '安全等级：L1~L4，影响工具调用权限与数据访问范围',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '状态：NORMAL（启用）/ DISABLED（禁用），管理员控制工具可用性',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `sbx_base_image` (
  ...
  `image_code` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '镜像编码',
  `image_name` varchar(128) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '镜像名称（如 python-datascience）',
  `description` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '描述（包含哪些包/环境）',
  `registry_type` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'DOCKER_HUB' COMMENT '镜像仓库类型（DOCKER_HUB/HARBOR）',
  `registry` varchar(256) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'docker.io' COMMENT '镜像仓库地址',
  `repository` varchar(256) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '镜像仓库路径（如 library/python）',
  `tag` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'latest' COMMENT '镜像标签',
  `digest` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '镜像 SHA256 摘要',
...
CREATE TABLE `sbx_instance` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `instance_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '实例唯一标识，UUID字符串，用于全链路追踪',
  `pool_id` bigint DEFAULT NULL COMMENT '所属池ID，关联sbx_pool.id',
  `tenant_id` bigint DEFAULT NULL COMMENT '占用方租户ID，记录当前分配的租户，实现跨租户隔离',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '实例状态：OCCUPIED（占用中）/ IDLE（空闲）/ ABNORMAL（异常）',
  `snapshot_id` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'AgentScope 沙箱快照ID，用于进程重启后恢复沙箱状态',
  `snapshot_oss_key` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '沙箱快照 OSS 对象键，MinIO 中存储的快照 tar 包路径',
  ...
  `isolation_scope` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '隔离作用域：USER / AGENT / GLOBAL / SESSION',
  `slot_key` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '沙箱槽位键，由 IsolationScope+业务主键合成，决定 slot 复用粒度',
  ...
  `session_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '占用会话ID，关联sess_session.session_id，记录实例绑定的会话',
  `pod_name` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Pod名称，Kubernetes中运行的容器名，由编排系统生成',
  `namespace` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '命名空间，Kubernetes命名空间，实例运行的逻辑隔离空间',
  ...
  `recycle_strategy` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '最近回收策略',
  `resource_fingerprint` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'resource fingerprint',
  PRIMARY KEY (`id`),
...
CREATE TABLE `sbx_lease` (
  ...
  `lease_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '租约唯一标识，UUID字符串',
  `instance_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '实例ID，关联sbx_instance',
  `session_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '持有租约的会话ID',
  `slot_key` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '槽位键',
  `expire_at` datetime DEFAULT NULL COMMENT '租约过期时间',
  `status` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '租约状态：ACTIVE / EXPIRED / RELEASED',
  ...
  KEY `idx_tenant_status` (`tenant_id`,`status`)
) ENGINE=InnoDB AUTO_INCREMENT=18 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='沙箱租约表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `sbx_operation_log` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `instance_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '沙箱实例ID',
  `pool_id` bigint DEFAULT NULL COMMENT '所属池ID',
  `tenant_id` bigint DEFAULT NULL COMMENT '租户ID',
  `operation_type` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '操作类型: ALLOCATE/RELEASE/RECLAIM/DESTROY/REPAIR/HEARTBEAT/FORCE_DESTROY',
  `source` varchar(16) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'RUNTIME' COMMENT '操作来源: RUNTIME/ADMIN/SYSTEM',
  `from_status` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '状态变更前',
  `to_status` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '状态变更后',
  `user_id` bigint DEFAULT NULL COMMENT '操作用户ID',
  `agent_id` bigint DEFAULT NULL COMMENT '操作Agent ID',
  `session_id` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '操作会话ID',
  `slot_key` varchar(256) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '槽位键',
  `success` tinyint NOT NULL DEFAULT '1' COMMENT '是否成功: 1=成功 0=失败',
  `error_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '错误码（失败时填写）',
  `error_message` varchar(1024) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '错误消息（失败时填写）',
...
) ENGINE=InnoDB AUTO_INCREMENT=2093999433634373634 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='沙箱操作审计日志';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `sbx_pool` (
  ...
  `pool_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '池编码',
  `namespace` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'K8s 命名空间名（aegis-sbx-t{tenantId}-{type}）',
  ...
  `pool_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '池名称，长度不超过128，如"轻量脚本池"、"强力计算池"',
  `pool_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '沙箱池类型：LIGHT（通用轻量）/ STANDARD（标准执行）/ HEAVY（重型计算）/ ISOLATED（高安全隔离）/ DEBUG（临时调试）',
  `applicable_scene` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '适用场景，长度不超过512，说明该池适用的智能体场景',
  `network_policy` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '网络策略：ISOLATED（隔离）/ RESTRICTED（限制出站）/ NO_EXTERNAL（禁止外网）',
  `cpu_limit` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'CPU限制，单实例最大CPU核数，如0.5、1、2',
  `mem_limit_mb` int DEFAULT NULL COMMENT '内存限制，单位MB，单实例最大内存',
  `disk_limit_gb` int DEFAULT NULL COMMENT '磁盘限制，单位GB，单实例最大磁盘空间',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '状态：ENABLED（启用）/ DISABLED（禁用）/ MAINTAINING（维护中）',
...
CREATE TABLE `sec_hitl_history` (
  ...
  `session_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '会话ID，关联sess_session.session_id，审批所在会话',
  `action` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '审批动作：APPROVE（通过）/ REJECT（拒绝）/ MODIFY（修改）/ TIMEOUT（超时）',
  `operator_user_id` bigint DEFAULT NULL COMMENT '操作人用户ID，关联org_user.id，执行审批的用户',
  `operator_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '操作人姓名，冗余存储便于审计列表展示',
  `detail` text COLLATE utf8mb4_unicode_ci COMMENT '操作详情，JSON字符串，记录审批意见、修改内容等',
...
CREATE TABLE `sec_hitl_node` (
  ...
  `node_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '节点名称，长度不超过128，标识审批环节',
  ...
  `approver_role` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '审批角色，当approverUserId为空时按角色匹配审批人',
  `sla_hours` int DEFAULT NULL COMMENT 'SLA时限，单位小时，审批超时时间，取值范围1-168',
  `timeout_strategy` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '超时策略：AUTO_APPROVE（自动通过）/ AUTO_REJECT（自动驳回）/ ESCALATE（升级处理）',
...
CREATE TABLE `sec_mask_rule` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `data_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '数据类型：PHONE（手机号）/ ID_CARD（身份证）/ BANK_CARD（银行卡）/ EMAIL（邮箱）/ IP（IP地址）/ CUSTOM（自定义）',
  `regex` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '识别正则，匹配敏感数据的正则表达式',
  `mask_way` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '脱敏方式：MIDDLE4（中间4位*）/ KEEP_HEAD_TAIL（保留首尾）/ KEEP_LAST4（保留后4位）/ ALL（全部替换）/ HASH（哈希脱敏）',
  `example` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '脱敏示例，展示脱敏前后对比，如"138****1234"',
...
CREATE TABLE `sec_outbound_policy` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `policy_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '策略类型：WHITELIST_DOMAIN（白名单域名）/ BLACKLIST_IP（黑名单IP）',
  `domain` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '域名，控制访问的目标域名，如api.example.com，支持通配符*.example.com',
  `ip_cidr` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'IP CIDR，控制访问的目标IP段，如192.168.1.0/24',
  `port_limit` int DEFAULT NULL COMMENT '端口限制，允许访问的端口号',
  `applicable_scope` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '适用范围：ALL（全部）/ AGENT（指定智能体）/ DEPT（指定部门）',
  ...
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '策略描述，长度不超过512，说明策略目的与适用场景',
...
CREATE TABLE `sec_sensitive_word` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `word` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '敏感词内容，长度不超过128',
  `category` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '敏感词分类：GENERAL（通用）/ INDUSTRY（行业）/ ENTERPRISE（企业自定义）/ PRIVACY（个人隐私）',
  `match_mode` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '匹配模式：EXACT（精确）/ FUZZY（模糊）/ REGEX（正则）',
  `action` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '处理动作：BLOCK（拦截）/ REPLACE（替换）/ MARK（标记）',
  `replace_text` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '替换文本，当action为REPLACE时使用的替换内容',
  `scope` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '适用范围：INPUT（用户输入）/ OUTPUT（模型输出）/ TOOL_RESULT（工具返回）',
...
CREATE TABLE `sec_tool_policy` (
  ...
  `action` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '处理动作：ALLOW（允许）/ APPROVE（需审批）/ REJECT（拒绝）',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '策略描述，长度不超过512，说明策略目的与适用场景',
  `enabled` tinyint(1) DEFAULT NULL COMMENT '是否启用，true生效，false暂停策略',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  ...
  `deleted` tinyint DEFAULT '0' COMMENT '逻辑删除：0未删除 1已删除',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_tool_policy` (`tenant_id`,`tool_type`,`security_level`),
...
CREATE TABLE `sess_artifact` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `artifact_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '业务ID (UUID)',
  `session_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '会话ID',
  `agent_id` bigint NOT NULL COMMENT '智能体ID',
  `msg_seq` int DEFAULT NULL COMMENT '关联的消息序号',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `user_id` bigint DEFAULT NULL COMMENT '用户ID',
  `name` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '产物名称 (文件名)',
  `type` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '产物类型: docx/pdf/excel/ppt/image/code/url/other',
  `storage_ref` varchar(512) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '存储引用 (MinIO objectKey / 外部URL)',
  ...
  `parent_artifact_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '父产物ID (版本链)',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '描述/备注',
  ...
  KEY `idx_create_time` (`create_time`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='会话产物表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `sess_message` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `session_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '会话ID，关联sess_session.session_id，消息所属会话',
  `message_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '消息类型：USER（用户消息）/ ASSISTANT（助手消息）/ TOOL_CALL（工具调用）/ TOOL_RESULT（工具结果）',
  `content` text COLLATE utf8mb4_unicode_ci COMMENT '消息内容，文本类型消息的正文，最长32KB',
  `reasoning` text COLLATE utf8mb4_unicode_ci COMMENT '推理过程，智能体思维链（CoT）内容，可选，用于透明化展示',
  `tool_call_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '工具调用ID，当messageType为TOOL_CALL时生成，用于关联调用与结果',
  `tool_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '工具名称，调用的工具标识，关联res_tool.tool_code',
...
CREATE TABLE `sess_session` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `session_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '会话唯一标识，UUID字符串，用于全链路追踪',
  `agent_state_session_key` varchar(256) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'AgentScope TenantSessionKey 标识（tenantId:userId:agentType:agentId:sessionId）',
  `agent_id` bigint DEFAULT NULL COMMENT '智能体ID，关联agent_def.id，会话绑定的智能体',
  `agent_type` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '智能体类型：UNIVERSAL/APPLICATION/SYSTEM',
  `agent_version` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '智能体版本，会话创建时的智能体版本号，保证会话可复现',
  `user_id` bigint DEFAULT NULL COMMENT '用户ID，关联org_user.id，会话所有者',
  `title` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '会话标题，长度不超过128，默认取首条消息摘要，用户可修改',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '会话状态：STARTED→THINKING→TOOL_CALLING→OUTPUTTING，异常为EXCEPTION，结束为ENDED',
  `sandbox_id` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '沙箱实例ID，关联sbx_instance.id，会话绑定的运行时沙箱',
...
CREATE TABLE `ten_quota` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint DEFAULT NULL COMMENT '租户ID，关联Tenant主键，1:1关系',
...
CREATE TABLE `ten_tenant` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '租户编码，全局唯一，业务可读标识，用于URL与配置引用，创建后不可修改',
  `tenant_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '租户名称，展示用，可重复，支持修改',
  `tenant_type` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '租户类型：HQ（集团总部）/ SUBSIDIARY（子公司）/ DIVISION（事业部），决定默认配额档位',
  `status` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '租户状态：NORMAL（正常）/ FROZEN（冻结），冻结后拒绝新建会话与资源操作',
  `contact_name` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '租户联系人姓名，用于运营对接',
  `contact_phone` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '租户联系人电话，用于运营对接与紧急通知',
  `expire_time` datetime DEFAULT NULL COMMENT '租户有效期截止时间，过期自动停用，null表示长期有效',
  `remark` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '备注，运营自定义描述信息',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `ten_usage` (
  `id` bigint NOT NULL COMMENT '主键ID（雪花ID）',
  `tenant_id` bigint DEFAULT NULL COMMENT '租户ID，关联Tenant主键',
